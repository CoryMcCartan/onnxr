...
test_that("input validation catches wrong dimensions", {
    ...
    expect_error(onnx_run(sess, bad_input), "dimension 2 is 4 but model expects 3")

    # Wrong number of dimensions (3D instead of 2D)
    bad_input_3d <- array(1:24, dim = c(2, 3, 4))
    expect_error(onnx_run(sess, bad_input_3d), "3 dimensions but model expects 2")

    # No dim attribute
    expect_error(onnx_run(sess, 1:6), "must have a dim attribute")
})
...
test_that("integer input produces same results as numeric input", {
    ...
    result_num <- onnx_run(sess, input_num, simplify = TRUE)
    result_int <- onnx_run(sess, input_int, simplify = TRUE)
...
test_that("named input matching works", {
    ...
    result_named <- onnx_run(sess, X = input_mat, simplify = TRUE)
    result_positional <- onnx_run(sess, input_mat, simplify = TRUE)

    expect_equal(result_named, result_positional)

    # Bad input name errors
    expect_error(onnx_run(sess, bad_name = input_mat), "Unknown input name")
})
